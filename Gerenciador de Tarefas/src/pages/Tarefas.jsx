...
function Tarefas() {
    ...
    async function adicionarTarefa() {
        const token = localStorage.getItem("token");
        if (tarefa !== "") {
            await axios.post("http://localhost:5000/tarefas", { tarefa }, {
                headers: {
    ...
    async function salvarEdicao(index) {
        ...
        if (tarefaEditada.tarefa !== "") {
            await axios.put(`http://localhost:5000/editar/${tarefaEditada.id}`, { tarefa: tarefaEditada.tarefa }, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
                }
    ...
    async function excluirTarefa(index) {
        const token = localStorage.getItem("token");
        await axios.delete(`http://localhost:5000/deletar/${tarefas[index].id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        chamarTarefas();
    }
    ...
    async function riscar(index) {
        try {
            ...
            await axios.put(`http://localhost:5000/riscar/${tarefaAtualizada.id}`, { concluida: tarefaAtualizada.concluida }, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
                }
